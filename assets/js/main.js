...
function movieSearch() {
    ...
    //$('#randomNight').append('<p> Movie Night </p>');
    ...
    var randomResult = searchWords[Math.floor(Math.random()*searchWords.length)];
        ...
        $.ajax({
            ...
            method: "GET"
                ...
                $('#randomNight').append('<p> Movie Night </p>');
                $('#randomNight').append('<p><strong> Movie Suggestions: </strong><br>' + response.Search["2"].Title +'<br>'
                                        + response.Search["4"].Title + '<br>'
                                        + response.Search["8"].Title + '<br></p>'); 
        });
};
...
function bookSearch(){ 
    ...
    //$('#randomNight').append('<p> Book Night </p>');
    ...
    var url = "https://api.nytimes.com/svc/books/v3/lists/best-sellers/history.json";
                    ...
                    $.ajax({
                        ...
                        $('#randomNight').append('<p><strong> Book Suggestions: </strong><br>' + result.results["4"].title + ' by ' + result.results["4"].author + '<br>'
                                                + result.results["6"].title + ' by ' + result.results["6"].author + '<br>' 
                                                + result.results["8"].title + ' by ' + result.results["8"].author + '<br></p>'); 
                    }).fail(function(err) {
                        throw err;
                });
};
...
//Beer Search
function beerSearch(){
    var searchBeers = ['Samuel Adams Boston Lager', 'New Belgium Trippel', 'Sierra Nevada Pale Ale', 'Rogue Dead Guy Ale', 
                        'Stone Porter', 'Guiness Draught', 'New Belgium Fat Tire', 'Yuenling Lager', 'Red Oak Amber', 'Angry Orchard Crisp Apple', 
                        'Guiness Blonde', 'Yuengling Black and Tan', 'Dos Equis Amber', 'Stone Arrogant Bastard Ale', 
                        'New Belgium Voodoo Ranger', 'Fat Tire Belgian White', 'New Belgium Pilsner', ];

    var randomBeerResult = searchBeers[Math.floor(Math.random()*searchBeers.length)];

    console.log(randomBeerResult);

    $('#randomNight').append('<h6> Beer: </h6> <p>' + randomBeerResult + '</p>');
